...
public class Connector implements ca.uqac.lif.petitpoucet.circuit.Connector
{
	...
	/**
	 * Connects two processors, the latter of which is to be called
	 * after being connected.
	 * @param p The first processor
	 * @param c The second processor, wrapped into a {@link CallAfterConnect}
	 * object
	 * @return The second processor
	 */
	public static Processor connect(Processor p, CallAfterConnect c)
	{
		connect(p, c.getProcessor());
		c.call();
		return c.getProcessor();
	}
	...
	/**
	 * An object that proxies access to a processor's output ports.
	 * This object is returned by {@link Processor#getOut()}.
	 * @since 3.14
	 */
	public static final class OutputPorts
	{
		private final Processor m_p;

		public OutputPorts(Processor p)
		{
			m_p = p;
		}

		/** Groovy maps p.out[i] to p.out.getAt(i). */
		public SelectedOutputPipe getAt(int index)
		{
			return new SelectedOutputPipe(m_p, index);
		}
	}

	/**
	 * An object that proxies access to a processor's input ports.
	 * This object is returned by {@link Processor#getIn()}.
	 * @since 3.14
	 */
	public static final class InputPorts
	{
		...
		public InputPorts(Processor p)
		{
			m_p = p;
		}

		/** Groovy maps p.in[i] to p.in.getAt(i). */
		public SelectedInputPipe getAt(int index)
		{
			return new SelectedInputPipe(m_p, index);
		}
	}
