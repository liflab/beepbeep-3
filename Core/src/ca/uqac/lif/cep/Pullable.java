...
public interface Pullable extends Iterator<Object>, Iterable<Object>
{
   ...
   * happens, the possible return values are:
   * <ul>
   * <li>If one of the inputs answers "no" when called for {@link #hasNextSoft()},
   * returns "no"</li>
   * <li>If one of the inputs answers "maybe" when called for
   * {@link #hasNextSoft()}, returns "maybe"</li>
   * <li>If all inputs answer "yes" when called for {@link #hasNextSoft()}, but no
   * event is produced with the given inputs, returns "maybe"</li>
  ...
  /**
   * Determines if an event can be pulled from the output, by trying "harder" than
   * {@link #hasNextSoft()}. Depending on what happens, the possible return values
   * are:
   * <ul>
   * <li>If one of the inputs answers "no" when called for {@link #hasNextSoft()},
   * returns "no"</li>
   * <li>If all inputs answer "yes" when called for {@link #hasNextSoft()}, and an
   * event is produced with the given inputs, returns "yes"</li>
   * <li>Otherwise, the method will keep calling {@link #hasNext()} on the inputs
   * as long as they don't answer "no", and will try to produce an output event
   * until one is produced.</li>
   * <li>To avoid infinite looping, the method eventually gives up (and answers
   * "no") after some maximum number of repetitions is reached. This is configured
   * by the static field {@link Processor#MAX_PULL_RETRIES}.
   * </ul>
   * 
   * @return Whether a next event exists
   */
  ...
  /**
   * Starts the pullable. This may be useful for multi-threaded pullables.
   */
  public void start();

  /**
   * Stops the pullable. This may be useful for multi-threaded pullables.
   */
  public void stop();
  ...
  /**
   * Pullable object that throws an {@link UnsupportedOperationException} upon
   * every call to each of its methods (except {@link #getProcessor()}). This
   * object can be returned by processors to signal that they cannot be pulled.
   */
