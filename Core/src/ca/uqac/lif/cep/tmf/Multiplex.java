 ...
 * ("muxer") is an <i>n</i>:1 processor. However, contrarily to other <i>n</i>:1
 * processors, the multiplexer does not wait until all its <i>n</i> inputs
 * produced an event before doing something. It directly sends to its (single)
 * output whatever events come from any of its inputs.
 * <p>
 * The muxer provides the following guarantees:
 * <ul>
 * <li>All input events are sent to the output</li>
 * <li>All input events received at step <i>k</i> will be output before any
 * event received at step <i>k</i>+1</li>
 * <li>There is no guarantee as to the output ordering of events received at the
 * same step</li>
 * </ul>
 * <p>
 * In other words, the muxer provides a way to merge <i>n</i> input traces into
 * a single one, preserving the relative ordering of events coming from the same
 * input trace.
 * 
...
public class Multiplex extends Processor
{
   ...
   * notified of the end of trace or not. Used to determine if the Multiplexer
   * should notify its PushableOutput of the end of trace or not
   */
   ...
   * 
   * @param in_arity
   *          The input arity of the multiplexer. This is the number of input
   *          traces that should be merged together in the output
   */
