...
/**
 * Tracks the relationship between output events produced by processors, and the
 * input events that were used to compute them. {@link ca.uqac.lif.cep.Processor
 * Processor} objects can be associated to a tracker. When this is the case,
 * they <em>may</em> call the tracker, asking it to record a relationship
 * between events. The tracker can use this set of recorded relationships,
 * potentially coming from multiple processors, to reconstruct the chain of
 * computation leading to a particular event.
 * <p>
...
public interface EventTracker
{
   ...
   *          The index of the stream in the processor where this output event is
   *          produced
   * @param out_stream_pos
   *          The position in the stream corresponding to this output event
   */
  public void associateTo(int id, NodeFunction f, int out_stream_index, int out_stream_pos);

  /**
   * Associates an output event from a processor to an input event from that same
   * processor
   * 
   * @param id
   *          The ID of the processor
   * @param in_stream_index
   *          The index of the stream in the processor corresponding to the input
   *          event to associate
   * @param in_stream_pos
   *          The position in the input stream corresponding to this event
   * @param out_stream_index
   *          The index of the stream in the processor where the output event is
   *          produced
   * @param out_stream_pos
   *          The position in the stream corresponding to this output event
   */
  public void associateToInput(int id, int in_stream_index, int in_stream_pos, int out_stream_index,
      int out_stream_pos);

  /**
   * Associates an output event from a processor to another output event from that
   * same processor
   * 
   * @param id
   *          The ID of the processor
